...
__global__ void imageConvolution(float* input, float* output, float* filter) {
...
int main() {
    ...
    // Launch the imageConvolution kernel on the GPU
    imageConvolution << <gridSize, blockSize >> > (d_input, d_output, d_filter);
